...
def seed():
    print("[+] Limpiando y creando tablas en PostgreSQL...")
    ...
    try:
        ...
        # 2. Configuración del Productor y su Invernadero Unificado
        user = User(
            email="productor@hydroguard.io",
            password_hash=get_password_hash("password123"),
            full_name="Carlos Productor (HydroGuard Pro)",
        ...
        plan_base = db.query(Plan).filter(Plan.tier == PlanTierEnum.BASE).first()
        sub = UserSubscription(
            user_id=user.id,
            plan_id=plan_base.id,
            is_active=True
        )
        db.add(sub)
        ...
        # Dispositivo IoT ESP32 Central
        device = Device(
            greenhouse_id=gh.id,
            device_uid="ESP32-GH-PRO-001",
        ...
        for code, name, unit, val, s_min, s_max, c_min, c_max in env_sensors_data:
            ...
            db.add(thresh)
            db.add(TelemetryReading(sensor_id=s.id, device_id=device.id, value=val, recorded_at=datetime.now(timezone.utc)))

        # Sensores para cada Tanque (1 a 4)
        ...
        # Sensores Generador (Disponibles en el hardware, visibles según plan)
        gen_sensors = [
        ...
        # Sensores Solares (Disponibles en el hardware, visibles según plan)
        solar_sensors = [
